...
$(document).ready(function () {
	...
	$('#goodsOptionId1').change(function() {
		...
		$.ajax({
			...
			success : function(d) {
				//1. hidden으로 생성된 totalPrice의 값을 가져와서 숫자형태로 변경하여 초기화
				var totalPrice = parseInt($('#totalPrice').val());
				//2. 선택한 옵션의 value을 가져온다.
				...
				//4. 계산된 금액을 히든 폼과 priceDisplay(사용자가 보는 화면)에 입력하고 출력한다.
				$('#totalPrice').val(totalPrice);
				$('#priceDisplay').html("총상품금액 :" + totalPrice);
				
				//5.초기상태라면 기존문자열을 지워주고 옵션을 추가한다. 
